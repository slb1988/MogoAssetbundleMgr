...
public class InstanceGrid : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        m_starNumber = m_myTransform.parent.FindChild("InstanceChooseBodyGridStarList/" + "InstanceChooseBodyGridStarNumber").GetComponentsInChildren<UILabel>(true)[0];

        m_spStar = m_myTransform.parent.FindChild("InstanceChooseBodyGridStarList").FindChild("InstanceChooseBodyGridStar").FindChild("InstanceChooseBodyGridQuadStarBGUp").GetComponentsInChildren<UISprite>(true)[0];

        m_spStarDown = m_myTransform.parent.FindChild("InstanceChooseBodyGridStarList").FindChild("InstanceChooseBodyGridStar").FindChild("InstanceChooseBodyGridQuadStarBGDown").GetComponentsInChildren<UISprite>(true)[0];

        m_spGridIcon = m_myTransform.parent.FindChild("InstanceChooseBodyGridFG").GetComponentsInChildren<UISprite>(true)[0];
        m_spStarBG = m_myTransform.FindChild("InstanceChooseBodyGridStarBG").GetComponentsInChildren<UISprite>(true)[0];
        m_spBGUp = m_myTransform.FindChild("InstanceChooseBodyGridBGUp").GetComponentsInChildren<UISprite>(true)[0];

        m_goLockPart = m_myTransform.parent.FindChild("InstanceChooseBodyGridLock").gameObject;

        m_lblStarNum = m_myTransform.parent.FindChild("InstanceChooseBodyGridStarList/InstanceChooseBodyGridStarNumber").gameObject;
        m_lblBGText = m_myTransform.FindChild("InstanceChooseBodyGridBGText").gameObject;
    }
