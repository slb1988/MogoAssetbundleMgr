...
public class TestAssetbundleMainAsset : MonoBehaviour
{
    /// <summary>
    /// 存放被调用资源信息。（name为主键）
    /// </summary>
    ...
    private IEnumerator StartLoad(string fileName)
    {
        ...
        var go = w.assetBundle.Load(Utils.GetFileName(fileName));
    ...
    private void LoadAssetBundleInfo()
    {
        ...
        foreach (var build in buildInfo)
        {
            ...
            var path = Path.Combine(root.Parent.FullName, "Export\\0.0.0.1\\ExportedFiles\\").Replace("\\", "/");
            BuildMogoAssetBundleInfo(path, path + build.type);
            //}
        }
    ...
    private void BuildMogoAssetBundleInfo(string path, string folder)
    {
        ...
        foreach (var item in defFiles)
        {
            ...
            {
                var asset = GetAssetBundleInfo(se, path);
    ...
    public static List<BuildResourcesInfo> LoadBuildResourcesInfo()
    {
        ...
        foreach (SecurityElement item in xml.Children)
        {
            ...
            if (folders != null && folders.Children != null)
            {
